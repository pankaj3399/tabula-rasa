...
exports.getDueCards = async (req, res) => {
  try {
    ...
    console.log(`Getting due cards for user ${userId} with topic slug: ${topicSlug}`);
    ...
    let dueCards = user.cardProgress
      ...
      .sort((a, b) => {
        ...
        const aDaysOverdue = Math.max(0, (now - a.dueDate) / (24 * 60 * 60 * 1000));
        const bDaysOverdue = Math.max(0, (now - b.dueDate) / (24 * 60 * 60 * 1000));
      ...
      try {
        ...
        if (topicResponse.data.data && topicResponse.data.data.length > 0) {
          ...
          if (cardsResponse.data.data && cardsResponse.data.data.length > 0) {
            ...
            const topicCardIds = cardsResponse.data.data.map(card => parseInt(card.id));
            console.log(`Found ${topicCardIds.length} cards for topic ${topicSlug}:`, topicCardIds);
            
            // Filter due cards to only include cards from this topic
            if (dueCards.length > 0) {
              dueCards = dueCards.filter(cardId => topicCardIds.includes(parseInt(cardId)));
              console.log(`Filtered due cards for topic ${topicSlug}:`, dueCards);
            } else {
              // If no due cards but topic has cards, use the topic's cards
              dueCards = topicCardIds;
              console.log(`Using all topic cards since no due cards:`, dueCards);
            }
          } else {
  ...
  try {
    ...
    if (topicSlug) {
      ...
      if (topicResponse.data.data && topicResponse.data.data.length > 0) {
        ...
        console.log('Topic-specific cards response:', JSON.stringify(cardsResponse.data, null, 2));
        
        if (cardsResponse.data.data && cardsResponse.data.data.length > 0) {
          return res.json(cardsResponse.data);
        } else {
          console.log(`No cards found for topic ID ${topicId}. Falling back to regular card lookup.`);
        }
      } else {
...
exports.getTopicCards = async (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${cardsResponse.data.data.length} cards for topic: ${slug}`);
    ...
    console.error('Error fetching topic cards:', error.response ? error.response.data : error.message);
...
exports.getTopicCardsDueForReview = async (req, res) => {
  ...
  console.log(`[GetDueTopicCards] Request received for slug: "${slug}", userId: ${userId}, limit: ${limit}`);
  ...
  if (!slug) {
      ...
      return res.status(400).json({ error: 'Topic slug path parameter is required' });
  }
  ...
  if (limit) {
      ...
      if (!isNaN(parsedLimit) && parsedLimit > 0) {
          ...
          console.log(`[GetDueTopicCards] Applying session limit: ${sessionLimit}`);
           ...
           console.warn(`[GetDueTopicCards] Invalid limit query parameter received: "${limit}". Ignoring limit.`);
      }
  }
  ...
  if (!STRAPI_URL || !STRAPI_TOKEN) {
      console.error('[GetDueTopicCards] STRAPI_URL or STRAPI_TOKEN missing from environment variables.');
  ...
  try {
      ...
      const topicQueryString = qs.stringify({ filters: { slug: { $eq: slug } }, fields: ['id', 'title'] }, { encodeValuesOnly: true });
      console.log(`[GetDueTopicCards] Strapi Topic Query URL: ${STRAPI_URL}/api/topics?${topicQueryString}`);
      let topicId;
      let topicTitle = slug; // Default title to slug if fetch fails
      try {
          const topicResponse = await axios.get(`${STRAPI_URL}/api/topics?${topicQueryString}`, { headers: { Authorization: `Bearer ${STRAPI_TOKEN}` } });
          if (!topicResponse.data.data || topicResponse.data.data.length === 0) {
              console.error(`[GetDueTopicCards] Topic not found in Strapi with slug: ${slug}`);
              return res.status(404).json({ error: `Topic not found with slug: ${slug}` });
          }
          topicId = topicResponse.data.data[0].id;
           // Adjust based on whether your topics are flattened or use attributes
          topicTitle = topicResponse.data.data[0].attributes?.title || topicResponse.data.data[0].title || slug;
          console.log(`[GetDueTopicCards] Found topic ID: ${topicId}, Title: "${topicTitle}"`);
      } catch (topicError) {
           console.error(`[GetDueTopicCards] Error fetching topic from Strapi for slug ${slug}:`, topicError.message);
           return res.status(500).json({ error: 'Failed to fetch topic details from Strapi', details: topicError.message });
      }
      ...
      const cardsQueryString = qs.stringify(cardsQueryObject, { encodeValuesOnly: true });
      console.log(`[GetDueTopicCards] CONSTRUCTED Strapi Card Query URL: ${STRAPI_URL}/api/cards?${cardsQueryString}`);

      let allTopicCardsRaw = [];
      try {
          console.log(`[GetDueTopicCards] Executing Strapi card query...`);
          const cardsResponse = await axios.get(`${STRAPI_URL}/api/cards?${cardsQueryString}`, { headers: { Authorization: `Bearer ${STRAPI_TOKEN}` } });
          ...
          console.error(`[GetDueTopicCards] Error fetching cards from Strapi for topic ${topicId}:`, cardError.message);
          return res.status(500).json({ error: 'Failed to fetch card details from Strapi', details: cardError.message });
      }

      console.log(`[GetDueTopicCards] Found ${allTopicCardsRaw.length} total cards in Strapi response for topic ${slug}`);
      if (allTopicCardsRaw.length === 0) {
           console.log(`[GetDueTopicCards] No cards associated with topic ${slug} in Strapi response.`);
      ...
      console.log(`[GetDueTopicCards] Created progress map with ${progressMap.size} entries.`);
      ...
      const now = new Date();
      const dueOrNewCards = [];
      console.log(`[GetDueTopicCards] Filtering cards against current time: ${now.toISOString()}`);
      console.log(`[GetDueTopicCards] --- Start Card Filtering Loop (Processing ${allTopicCardsRaw.length} cards) ---`);
      ...
      for (const cardRaw of allTopicCardsRaw) {
          ...
          const cardData = { ...cardRaw }; // Use the raw card object directly
           ...
           if (!cardData.id) {
              console.warn(`[GetDueTopicCards] Skipping card from Strapi due to missing ID:`, cardRaw);
              continue;
          }
          ...
          const dueDate = progress && progress.dueDate ? new Date(progress.dueDate) : new Date(0);
          ...
          console.log(`  [Card Check] ID: ${cardIdStr.padEnd(5)} | Progress: ${progress ? 'Yes' : 'No '} | Due Date: ${dueDate.toISOString().padEnd(25)} | isNew: ${isNew.toString().padEnd(5)} | isDue: ${isDue.toString().padEnd(5)} ${isDue ? '<<< INCLUDED' : ''}`);
      ...
      console.log(`[GetDueTopicCards] Found ${dueOrNewCards.length} due or new cards after filtering.`);
      console.log(`[GetDueTopicCards] Included Card IDs (Pre-sort): [${dueOrNewCards.map(c => c.id).join(', ')}]`);
      ...
      if (dueOrNewCards.length > 0) {
           ...
           dueOrNewCards.forEach(c => console.log(`  - ID: ${c.id.toString().padEnd(5)} | Due: ${c.dueDate.toISOString()}`));
      }
      ...
      if (sessionLimit && finalSessionCards.length > sessionLimit) {
          ...
          console.log(`[GetDueTopicCards] Limited session cards to ${sessionLimit}.`);
          console.log(`[GetDueTopicCards] Final Session Card IDs (Post-limit): [${finalSessionCards.map(c => c.id).join(', ')}]`);
      } else {
          console.log(`[GetDueTopicCards] No limit applied or list shorter than limit.`);
      }

      // --- 8. Send Response ---
      console.log(`[GetDueTopicCards] Sending ${finalSessionCards.length} cards back to client.`);
      console.log(`--- [GetDueTopicCards] End ---`);
      res.json({ data: finalSessionCards }); // Send the final list

  } catch (error) {
      // Catch unexpected errors during the process
      console.error('[GetDueTopicCards] !!! UNEXPECTED Error during processing:', error.message);
      console.error(error.stack);
      res.status(500).json({ error: 'Internal server error processing due cards', details: error.message });
  }
};
...
exports.updateCardProgress = async (req, res) => {
  ...
  console.log('[UpdateCardProgress] Request Body:', JSON.stringify(req.body, null, 2));
  ...
  try {
    ...
    if (!userId || !cardId || quality === undefined || quality === null) {
      console.error('[UpdateCardProgress] Invalid input data:', { userId, cardId, quality });
      return res.status(400).json({ error: 'Missing or invalid userId, cardId, or quality' });
    }
    ...
    if (!user.cardProgress) {
        console.log('[UpdateCardProgress] Initializing cardProgress array for user.');
        user.cardProgress = [];
    }

    // --- Find Card Progress ---
    console.log(`[UpdateCardProgress] Searching for existing progress for card ID: ${cardId}`);
    let cardProgress = user.cardProgress.find(cp => cp.cardId === cardId.toString()); // Ensure comparison with string ID
    ...
    if (!cardProgress) {
      // --- Create New Progress ---
      console.log(`[UpdateCardProgress] No existing progress found for card ${cardId}. Creating new entry.`);
      ...
      console.log('[UpdateCardProgress] New progress entry pushed:', JSON.stringify(newProgressEntry, null, 2));
      ...
      console.log(`[UpdateCardProgress] Existing progress found for card ${cardId}. Updating.`);
      console.log('[UpdateCardProgress] Old Progress:', JSON.stringify(cardProgress, null, 2));
      ...
      // --- Apply SRS Algorithm ---
      console.log(`[UpdateCardProgress] Applying SRS logic with quality: ${qualityNum}`);
      let calculatedInterval;
      let calculatedEaseFactor;

      if (qualityNum >= 3) {
        console.log('[UpdateCardProgress] Quality >= 3 (Good Recall)');
        if (cardProgress.interval <= 0) { // Interval shouldn't be 0, but handle defensively
          calculatedInterval = 1;
          console.log(`[UpdateCardProgress] Interval was <= 0, set to: ${calculatedInterval}`);
        } else if (cardProgress.interval === 1) {
          calculatedInterval = 6;
          console.log(`[UpdateCardProgress] Interval was 1, set to: ${calculatedInterval}`);
        } else {
          calculatedInterval = Math.round(cardProgress.interval * cardProgress.easeFactor);
          console.log(`[UpdateCardProgress] Calculated interval: round(${cardProgress.interval} * ${cardProgress.easeFactor}) = ${calculatedInterval}`);
        }
        // Calculate new ease factor using SM-2 formula adaptation
        calculatedEaseFactor = cardProgress.easeFactor + (0.1 - (5 - qualityNum) * (0.08 + (5 - qualityNum) * 0.02));
        calculatedEaseFactor = Math.max(1.3, calculatedEaseFactor); // Ensure ease factor doesn't drop below 1.3
        console.log(`[UpdateCardProgress] Calculated ease factor: max(1.3, ${cardProgress.easeFactor} + (0.1 - (${5 - qualityNum}) * (0.08 + (${5 - qualityNum}) * 0.02))) = ${calculatedEaseFactor}`);

      } else {
        console.log('[UpdateCardProgress] Quality < 3 (Poor Recall)');
        calculatedInterval = 1; // Reset interval
        console.log(`[UpdateCardProgress] Interval reset to: ${calculatedInterval}`);
        // Decrease ease factor, minimum 1.3
        calculatedEaseFactor = Math.max(1.3, cardProgress.easeFactor - 0.2);
         console.log(`[UpdateCardProgress] Calculated ease factor: max(1.3, ${cardProgress.easeFactor} - 0.2) = ${calculatedEaseFactor}`);
      }
      ...
      console.log(`[UpdateCardProgress] Calculated new due date: ${now.toISOString()} + ${cardProgress.interval} days = ${cardProgress.dueDate.toISOString()}`);
    }
    ...
    res.json({ message: 'Card progress updated successfully', newProgress: cardProgress }); // Optionally return the updated progress

  } catch (error) {
    console.error('[UpdateCardProgress] !!! Error updating card progress:', error.message);
    console.error(error.stack); // Log stack trace for more details
    res.status(500).json({ error: 'Failed to update card progress', details: error.message });
  }
};
...
exports.setStudySessionSettings = async (req, res) => {
  try {
    ...
    await user.save();
    res.json({ message: 'Study session settings updated', settings: user.studySettings });
  } catch (error) {
