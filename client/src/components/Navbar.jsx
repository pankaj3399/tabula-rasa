      ...
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        <div className="flex items-center">
          <Link to="/" className="flex items-center text-2xl font-bold text-white">
            <svg className="w-10 h-10 mr-2" viewBox="0 0 24 24" fill="#6366F1">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" />
              <path d="M12 6c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6-2.69-6-6-6zm0 10c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4z" />
            </svg>
            Tabula Rasa
          </Link>
          <div className="ml-4 bg-[#6C2BD9] px-3 py-1 rounded-md text-sm font-medium">
            PA Exam Prep
          </div>
        </div>
          ...
          <div className="px-4 py-4 space-y-4">
            {navItems.map(({ to, label }) => (
              <Link
                ...
                className="block py-2 text-white hover:text-indigo-300 transition"
              >
                {label}
              </Link>
            ))}
            {!currentUser ? (
              <div className="space-y-4">
                <Link
                  to="/login"
                  onClick={toggleMobileMenu}
                  className="block py-2 border border-white text-white text-center rounded-lg hover:bg-white hover:text-[#0E0526] transition"
                >
                  Login
                </Link>
                <Link
                  to="/signup"
                  onClick={toggleMobileMenu}
                  className="block py-2 bg-indigo-500 text-white text-center rounded-lg hover:bg-indigo-600 transition"
                >
                  Sign Up
                </Link>
              </div>
